...
### 배송 현황
관리자가 `/admin` 의 **배송 관리**에서 단계(상품 준비중 → 배송 시작 → 배송중 → 배송 완료)와
택배사·송장번호를 입력하면, 고객의 주문 상세에 타임라인으로 바로 나타납니다.

주문 테이블에는 UPDATE 정책이 없어서(금액 조작 방지) 상태 변경은 `shoe_set_fulfillment`
함수로만 가능하고, 그 함수가 관리자 여부를 먼저 확인합니다. 실제 택배사와는 연동돼 있지 않습니다.
...
`lib/seed-data.ts` 가 카탈로그의 단일 원본입니다(브랜드 21개 · 상품 45개 · 무드 매핑 106건).
